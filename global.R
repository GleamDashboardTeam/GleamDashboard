...
mem_used()
# animalNumbers
labelFunct <- label_number(scale_cut = cut_short_scale(), accuracy =  0.01)
...
gleam4AnimalNames <- list ( "CTL" = "Cattle", "BFL" = "Buffalo", "SHP" = "Sheep", "GTS" = "Goats", "PGS"= "Pigs", "CHK"= "Chicken")
gleam4LPS <- list ( "GRS" = "Grassland", "MXD" = "Mixed", "LYR" = "Layer", "BRL" = "Broiler", "BCK"= "Backyard", 
                    "MED"= "Intermediate","IND"= "Industrial")

gleam4Herds <- list ( "DRY" = "Dairy", "BEF" = "Beef",  "PGS"= "Pigs", "CHK"= "Chicken")
...
makeSankeyColorsAllEmissionTypes  <- function (){
  ...
  nodeColors.dt <- rbindlist (list (nodeColors.dt, data.table (nodeName = c("ChickenHerd", "PigsHerd"), color = c("#CCD8D2" , "#F9C4B0" ))),use.names = T)
...
countries <- unlist(split(as.character(dd$ISO3), dd$COUNTRY))
 ...
 #<- factor(c("Yes", "No", "Other", "Yes", "No", "Other"))
#gleamRegions<-levels(gleamRegions)
#gleamRegions<-factor(gleamRegions, levels=c(sort(gleamRegions, decreasing=T)))

if (isPublicVersion == T) {
  countries <- NA
  allRegionCountries <- c (contRegions, gleamRegions, faoRegions, wbRegions, oecdRegions, eu27Regions, worldRegions)
  
}else{
  allRegionCountries <- c (contRegions, gleamRegions, faoRegions, wbRegions, oecdRegions, eu27Regions, worldRegions, countries)
