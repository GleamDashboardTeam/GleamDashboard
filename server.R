...
shinyServer <- function (input, output, session){
  ...
  #addClass(selector = "body", class = "sidebar-collapse") # enable this to collapse sidebar on startup
  
  
  # landing page events #################
  
  observeEvent(input$switch_tab_population, {
    updateTabItems(session, "sidebar", "tab_population")
  })
  observeEvent(input$switch_tab_production, {
    updateTabItems(session, "sidebar", "tab_production")
  })
  observeEvent(input$switch_tab_emissions, {
    updateTabItems(session, "sidebar", "tab_emissions")
  })
  observeEvent(input$switch_tab_emIntensity, {
  ...
  # 
  # #############################  input data section ##################################
  # rv <- reactiveValues(Params = NULL )
  # 
  # observeEvent(input$selectCountry, {
  #   if (startsWith (input$selectCountry ,"Annex5")== TRUE){
  #     cat ("ANNEX5\n")
  #     pData <- fread(regAvgParamA5.file)
  #     dmData.dt <- fread (dmAnnex5.file)
  #     
  #      pData[, Animal:= gleam4AnimalNames [ Animal] [[1]], by= seq_along(1:nrow (pData))]
  #     pData[, LPS:= gleam4LPS [ LPS] [[1]], by= seq_along(1:nrow (pData))]
  #     pData[, Herd:= gleam4Herds[Herd] [[1]], by= seq_along(1:nrow (pData))]
  #     
  #     
  #     #gleam4LPS
  #    # merge (pData, gleamAnimals.dt,by.x= "Animal",by.y= "animal")
  #     xx <- gsub ("Annex5_", "", input$selectCountry)
  #     pData <- pData [ REGION == xx]
  #     #pData [, Species:=  gleam4AnimalNames[ Animal] [[1]], by = seq_along(1:nrow (pData)) ]
  #     #fwrite (pData, "junk1.csv")
  #     # cat (nrow (Pvalues$df_data), "\n")
  #     #setnames (Pvalues$df_data, "REG_ANNEX5", "REGION")
  #     pData <- merge (pData, paramListSuppl.dt,by.x ="varName", by.y = "VarNameShort" )
  #     pData[, tableHeader:= paste0 (VarNameText, "<br/> [ ", Unit, " ]" )]  
  #     dmData.dt <- dmData.dt [REG_ANNEX5 == input$selectCountry]
  #     
  #     
  #   }else{
  #     #if (input$selectCountry == "World")
  #     pData <- fread(regAvgParamReg.file)
  #     pData <- pData [REGION == input$selectCountry]
  #     #pData <- pData [ REGION == xx]
  #     
  #     pData[, Animal:= gleam4AnimalNames [ Animal] [[1]], by= seq_along(1:nrow (pData))]
  #     pData[, LPS:= gleam4LPS [ LPS] [[1]], by= seq_along(1:nrow (pData))]
  ...
  #     cat ("XXX", nrow (pData),"\n")
  #     pData[, tableHeader:= paste0 (VarNameText, "<br/> [ ", Unit, " ]" )]  
  #     dmData.dt <- fread (dmFAORegion.file)
  #     dmData.dt <- dmData.dt [REGION == input$selectCountry]
  #   }
  #   rv$Params <- pData 
  #   rv$DMData <- dmData.dt
  #   
  ...
  output$inputDatSelAnimal <- renderUI({
    ...
    radioButtons ("inData_selSpecies", label= "Select species",inline = T, choices = anChoices, selected= anChoices[[1 ]])
  })
...
#   cat ("SelParams, ",selParams, "\n")
  ...
  # xx.w <- xx.w[ V1 != 0]
   ...
   ret <-  DT::datatable(xx.w,  caption = "Herd parameters used in GLEAM v3, weighted by population. Rounded to 3 significant digits. " ,
                         ...
                         options = list(dom = 'Brtip',
                                        ...
                                        buttons =
                                          list('copy', 'print', list(
                                            extend = 'collection',
                                            buttons = list(
                                              list(extend = 'csv', filename = paste0("GLEAMv3HerdParameters", Sys.Date())   ) ,
                                              list(extend = 'excel', filename = paste0("GLEAMv3HerdParameters" , Sys.Date()) ),
                                              list(extend = 'pdf', filename = paste0("GLEAMv3HerdParameters", Sys.Date())    )),
                                            text = 'Download'
                                          ))
                         )
 ...
 #  xx <-  paramData.dt[Animal %in%  c("Cattle")  &  varName %in% selParams & COUNTRY == "World"]
   ...
   setnames (xx.w,"COUNTRY", "Area")
  # xx.w <- xx.w[ V1 != 0]
   ret <-  DT::datatable(xx.w,  caption = "Manure management systems percentage, used in GLEAM v3, weighted by population. Rounded to 3 significant digits. " ,
                         extensions=c("Buttons",'Scroller'),escape = F,
                         #colnames = c("Emissions [ t CO2eq ]" = "Emissions", "Production [ t ]" = "Production", "Emission Intensity [ kg CO2eq / kg ]"= "Intensity"),
                         options = list(dom = 'Brtip',
                                        scrollY = 200,
                                        ...
                                        buttons =
                                          list('copy', 'print', list(
                                            extend = 'collection',
                                            buttons = list(
                                              list(extend = 'csv', filename = paste0("GLEAMv3ManureMS", Sys.Date())   ) ,
                                              list(extend = 'excel', filename = paste0("GLEAMv3ManureMS" , Sys.Date()) ),
                                              list(extend = 'pdf', filename = paste0("GLEAMv3ManureMS", Sys.Date())    )),      
                                            text = 'Download'
                                          ))
                         )
 ...
 output$inputDataLiveWeight <- DT::renderDataTable({
  ...
  # xx <-  paramData.dt[Animal == c("Cattle") &  VarName == "DryMatterIntake"]
   ...
   setnames (xx.w,"COUNTRY", "Area")
   ret <-  DT::datatable(xx.w,  caption = "Live weights used in GLEAM v3, weighted by population in kg. Rounded to 3 significant digits. " ,
                         extensions=c("Buttons",'Scroller'),escape = F,
                         #colnames = c("Emissions [ t CO2eq ]" = "Emissions", "Production [ t ]" = "Production", "Emission Intensity [ kg CO2eq / kg ]"= "Intensity"),
                         options = list(dom = 'Brtip',
                                        scrollY = 200,
                                        ...
                                        buttons =
                                          list('copy', 'print', list(
                                            extend = 'collection',
                                            buttons = list(
                                              list(extend = 'csv', filename = paste0("GLEAMv3LiveWeight", Sys.Date())   ) ,
                                              list(extend = 'excel', filename = paste0("GLEAMv3LiveWeight" , Sys.Date()) ),
                                              list(extend = 'pdf', filename = paste0("GLEAMv3LiveWeight", Sys.Date())    )),                                          
                                            text = 'Download'
                                          ))
                         )
 ...
 output$inputDryMatterIntakeSummary <- DT::renderDataTable({
  ...
  # xx <- dmIntake.dt[ Animal == "Cattle" & COUNTRY == "World"]
   ...
   ret <-  DT::datatable(xx.w,  caption = "Total feed dry matter intake calculated with GLEAM v3 [ tonnes ] ." ,
                         extensions=c("Buttons",'Scroller'),escape = F,
                         #colnames = c("Emissions [ t CO2eq ]" = "Emissions", "Production [ t ]" = "Production", "Emission Intensity [ kg CO2eq / kg ]"= "Intensity"),
                         options = list(dom = 'Brtip',
                                        scrollY = 200,
                                        ...
                                        buttons =
                                          list('copy', 'print', list(
                                            extend = 'collection',
                                            buttons = list(
                                              list(extend = 'csv', filename = paste0("GLEAMv3DryMatterIntake", Sys.Date())   ) ,
                                              list(extend = 'excel', filename = paste0("GLEAMv3DryMatterIntake" , Sys.Date()) ),
                                              list(extend = 'pdf', filename = paste0("GLEAMv3DryMatterIntake", Sys.Date())    )),                                          
                                            text = 'Download'
                                          ))
                         )
  ...
  output$report_caption  <- renderText({ 
    ...
    # if (input$selectCountry %in% c(names (SpecialRegions))){
    #   ret <-  paste("","<h3>","Download region report for the", names(regionNamesISO) [ regionNamesISO == input$selectCountry], "region for 2015</h3>") 
    # }
    # if (input$selectCountry == "World") {
    #   ret <-  paste("","<h3>","Download world summary report for 2015</h3>") 
    #   
    # }
    # ret
  ...
  output$emIntensity_downloadPlot <- downloadHandler(
    filename = function(){paste("GLEAMv3_EMIntensity_", input$selectCountry,'.png',sep='')},
    content = function(file){
                            ...
                            options = list(dom = 'Bfrtip',
                                           ...
                                           buttons =
                                             list('copy', 'print', list(
                                               extend = 'collection',
                                               buttons = list(
                                                 list(extend = 'csv', filename = paste0("GLEAMv3Results", Sys.Date())   ) ,
                                                 list(extend = 'excel', filename = paste0("GLEAMv3Results", Sys.Date())   ) ,
                                                 list(extend = 'pdf', filename = paste0("GLEAMv3Results", Sys.Date())   ) ),
                                               text = 'Download'
                                             ))
                            )
 ...
 "), trigger = 'hover')
    ...
    shinydashboard::valueBox(
      ...
      subtitle = tags$p("Buffaloes", style = "font-size: 150%;"),
 ...
 "), trigger = 'hover')
    totAn <- allData.dt[VarName == "AnimalNumbers" & ISO3== input$selectCountry & Animal == "Chicken", sum (V1, na.rm = T)]
    #totAn <- allData.dt[VarName == "AnimalNumbers" & COUNTRY== "World" & Animal == "Chicken", sum (V1, na.rm = T)]
    ...
    shinydashboard::valueBox(
      value = tags$p(xx, style = "font-size: 100%;"),
      subtitle = tags$p("Chicken", style = "font-size: 150%;"),
  ...
  output$infoProdMeatTotal <- renderValueBox({
    ...
    xx <- labelFunctProd(totAn)
    shinydashboard:: valueBox(xx,
                              
                              "Tonnes of meat",  icon = NULL,
                              color = "blue", width = 2
    )
  })
  ...
  output$infoProdProteinTotal <- renderValueBox({
    ...
    shinydashboard:: valueBox(xx,
                              ...
                              "Tonnes of animal protein",  icon = NULL,
                              color = "blue", width =2
    )
  })
  # 
  output$infoEmissionsTotal <- renderValueBox({
    ...
    xx <- labelFunctEm(totEm)
    ret <-  shinydashboard:: valueBox(xx,
                                      HTML(paste0("Total emissions [ tonnes CO",tags$sub("2")),"eq ]" ),  icon = NULL,
                                      color = "blue",width =2
    )
  ...
  output$infoEmissionsTotFrac <- renderValueBox({
    ...
    labelFunctEm <- label_number(scale_cut = cut_short_scale(), accuracy =  0.1)
    
    
    xx <- labelFunctEm(frac * 100)
    #frac <- 99.9
    ret <-  shinydashboard:: valueBox(xx,
                                      HTML(paste0("percent of total global GHG emissions" )),  icon = NULL,
                                      color = "blue",width =2
  ...
  output$infoEmissionsRank <- renderValueBox({
    ...
    xx <- labelFunctEm(totEm)
    ret <-  shinydashboard:: valueBox(xx,
                                      HTML(paste0("Direct emissions [ tonnes CO",tags$sub("2")),"eq ]" ),  icon = NULL,
                                      color = "blue",width =2)
    ...
    # ret <- shinydashboard::valueBox(formatC(totEm, format = "f", big.mark = ",", digits =0),
    #                                  "Direct emissions [Tons of CO2 eq]",  icon =NULL,
    #                                  color = "blue")
  ...
  output$infoIntensityMeat <- renderValueBox({
    ...
    # totAn <- xx[ COUNTRY== 'World']$Intensity
    ...
    #print (    
    #totAn <- getEmissionIntensity  ("Production", input$selectCountry, c(Monogastrics, Ruminants) ,unique (allData.dt$LPS), c("Eggs"), EmissionSources,"Weight")
    #  print (xx$Intensity)
    ...
    #shinydashboard:: valueBox(sprintf ("%.2f ", totAn,
    
    #                              HTML(paste("kg CO", tags$sub(2), "per kg meat protein")
    #                              
    #                           ,
    #                            icon = NULL,
    #                            color = "blue", width =2 
  ...
  output$infoIntensityEggs <- renderValueBox({
    ...
    # shinydashboard:: valueBox(sprintf ("%.2f ", totAn),
    #                           tags$div(
    #                             HTML(paste("kg CO", tags$sub(2), sep = "")),
    #                             HTML(paste("eq per kg egg protein"))
    #                           ),
    # 
    #                           icon = NULL,
    #                           color = "blue", width = 2
  ...
  addPopover(session, "infoHerdSizeBUF", "Number of buffaloes", content = 
               "Total number of buffaloes. Based on Gridded livestock of the world (GLW), 2015.", trigger = 'hover')
  ...
  addPopover(session, "infoHerdSizeCHK", "Number of chicken", content = 
               "Total number of chicken. Based on Gridded livestock of the world (GLW), 2015.", trigger = 'hover')
  ...
  addPopover(session, "infoEmissionsTotFrac", "Percentage of all GHG emission", content = 
               "Percentage on total global antropogenic GHG emissions, 2015. ", trigger = 'hover')
  ...
  output$EmIntensityCommodity <- renderUI({
    ...
    # curCom <- unique (allData.dt[ VarName== "Production"& Animal %in% input$selAnimals & LPS %in% input$select_LPS_emIntensity]$Item)
  ...
  output$EmIntensity_emSource <- renderUI({
    ...
    # curCom <- c("Total protein", "Milk", "Meat", "Eggs", "per animal") 
    #checkboxGroupInput("select_Comm_emSource", "Emission source", choices = c( curCom), inline = T, selected= curCom)
  ...
  output$emIntensityBar <- renderPlotly({
    ...
    #req(input$int_selLPS)
    #req(input$selNodes)
    #req(input$select_Comm_emIntensity)
    #req(input$emIntensityBySource)
    #req(input$emIntensityBySource)
    ...
    # selNodes <- c("Animal")
    #bySource <- F
    #selSources <-c("Feed-N2O", "EntericFermentation")
    GWPCH4 <- GWPSets[[input$selectGWPSet ]]$CH4
    GWPN2O <- GWPSets[[input$selectGWPSet ]]$N2O
  ...
  # output$emIntensityFrequencyMono <- renderPlot({
  #   cat (input$intSelectLPSMono, "\n")
  #   cat (input$inOuputSelect, "\n")
  #   cat (input$inSelectMonogastrics, "\n")
  #   plot_emissionIntensityBoxplot (input$selectCountry,input$inOuputSelect, input$inSelectMonogastrics,"ALL" , input$intSelectLPSMono, FALSE )
  #   #function (selCountry, selProduct, selAnimals, selLPS, freeScale = F
  # })
  # output$emIntensityFrequencyRumin <- renderPlot({
  #   cat (input$intSelectLPSRumin, "\n")
  #   cat (input$inOuputSelect, "\n")
  #   cat (input$inSelectRuminants, "\n")
  #   cat ("SelHerd", input$inSelRuminantsHerdType, "\n")
  #   
  #   plot_emissionIntensityBoxplot (input$selectCountry,input$inOuputSelect, input$inSelectRuminants, input$inSelRuminantsHerdType, input$intSelectLPSRumin, FALSE)
  #   
  #   #return (empty_plot("Coming soon"))
  #   #plot_emissionIntensityBoxplot (input$selectCountry,input$inOuputSelect, input$inSelectRuminants, FALSE )
  #   
  # })
  
  
  # downloadable tables @###############################################################################################
  # output$tableIntensityRumin <-  DT::renderDataTable( makeDownloadTable ("EmissionsIntensity", Ruminants),  server = FALSE,
  #                                                    
  #                                                    
  ...
  # DT::renderDataTable ( makeDownloadTable ("EmissionsIntensity", Ruminants),  server = FALSE,
  #                       
  #                       
  #                       extensions = c("Buttons"), 
  #                       options = list(dom = 'Bfrtip',
  #                                      buttons = c('copy', 'csv', 'excel', 'pdf', 'print')
  #                       ))
  #    output$tableIntensityAll <-  DT::renderDataTable( makeDownloadTable ("EmissionsIntensity", c(Monogastrics, Ruminants)),  server = FALSE,
  #                                                       
  #                                                       
  #                                                       extensions = c("Buttons"), 
  #                                                       options = list(dom = 'Bfrtip',
  ...
  #   
  #   
  # output$tableEmissionsRumin <- DT::renderDataTable ( makeDownloadTable ("Emissions", Ruminants),  server = FALSE,
  #                                                     
  #                                                     
  #                                                     extensions = c("Buttons"), 
  #                                                     options = list(dom = 'Bfrtip',
  ...
  # 
  # 
  # output$tableEmissionsAll <- DT::renderDataTable ( makeDownloadTable ("Emissions", c(Monogastrics, Ruminants)),  server = FALSE,
  #                                                     
  #                                                     
  #                                                     extensions = c("Buttons"), 
  #                                                     options = list(dom = 'Bfrtip',
  ...
  # output$tableEmissionsMono  <- DT::renderDataTable ( makeDownloadTable ("Emissions", Monogastrics)
  #                                                     ,  server = FALSE,
  #                                                     
  #                                                     
  #                                                     extensions = c("Buttons"), 
  ...
  #                                                                    buttons = c('copy', 'csv', 'excel', 'pdf', 'print')
  #                                                     ))
  # output$tableAnimalNumbers<- DT::renderDataTable ( makeDownloadTable ("AnimalNumbers", c(Monogastrics, Ruminants)) 
  #                                                        ,  server = FALSE,
  #                                                        
  #                                                        filter= "top",
  #                                                        extensions = c("Buttons"), 
  ...
  # 
  # 
  # output$tableAnimalNumbersMono <- DT::renderDataTable ( makeDownloadTable ("AnimalNumbers", Monogastrics) 
  #                                                        ,  server = FALSE,
  #                                                        
  #                                                        
  #                                                        extensions = c("Buttons"), 
  ...
  #                                                                       buttons = c('copy', 'csv', 'excel', 'pdf', 'print')
  #                                                        ))
  # output$tableAnimalNumbersRumin <- DT::renderDataTable ( makeDownloadTable ("AnimalNumbers", Ruminants) 
  #                                                         ,  server = FALSE,
  #                                                         
  #                                                         
  #                                                         extensions = c("Buttons"), 
  ...
  #                                                                        buttons = c('copy', 'csv', 'excel', 'pdf', 'print')
  #                                                         ))
  #  output$tableProductionMono <- DT::renderDataTable ( makeDownloadTable ("Production", Monogastrics) 
  #    ,  server = FALSE,
  #    
  #    
  #    extensions = c("Buttons"), 
  ...
  #                   buttons = c('copy', 'csv', 'excel', 'pdf', 'print')
  #    ))
  #   output$tableProductionRumin  <- DT::renderDataTable ( makeDownloadTable ("Production",Ruminants),  server = FALSE,
  #                                                         
  #                                                         
  #                                                         extensions = c("Buttons"), 
  #                                                         options = list(dom = 'Bfrtip',
  ...
  #                                                         )) 
  # 
  # output$SankeyNetwork3D_ProductionRumin <- renderSankeyNetwork({
  #   nodes2show <- input$pr_selectSankeyNodesRumin
  #   selAnimals <-  input$inSelectRuminantsProduction
  #   nodes2show <- c( nodes2show)
  #   cat (nodes2show,"\n")
  #   p <- makeSankeyDataProduction(nodes2show, selAnimals, input$selectCountry)
  #   p
  # })
  # output$SankeyNetwork3D_ProductionMono <- renderSankeyNetwork({
  #   nodes2show <- input$pr_selectSankeyNodesMono
  #   selAnimals <-  input$inSelectMonogastricsProduction
  #   nodes2show <- c( nodes2show)
  #   #cat (selAnimals,"\n")
  #   p <- makeSankeyDataProduction(nodes2show, selAnimals, input$selectCountry)
  #   p
  ...
  # output$animalNumberRuminants <- renderPlotly({
  #   #data2plot <- allData.dt[VarName == "AnimalNumbers"  & COUNTRY ==   &  Animal %in%   & HerdType %in%  ]
  #   fig <- plotBarChartAnimalNumbers (input$selectCountry,input$inSelectAnimalsRuminants,input$inSelectAnimalsRuminantsHerdType ) 
  #   return (fig)
  # })
  # 
  # output$animalNumberMonogastrics <- renderPlotly({
  #   #data2plot <- allData.dt[ VarName == "AnimalNumbers" &  COUNTRY == input$selectCountry  &  Animal %in%  input$inSelectAnimalsMonogastrics  ]
  #   fig <- plotBarChartAnimalNumbers (input$selectCountry,input$inSelectMonogastrics, input$inSelectMonogastrics ) 
  #   
  ...
  output$emissionBarChart <- renderPlotly({
    ...
    #
    emissionsCntry.dt[, label:= paste (Animal, ", ", ProdSys, ":", sprintf ("%.3f", V1/1e9), "million tons CO2eq")]
  ...
  output$treeMapEmissions <- renderPlot({
    ...
    #fig <- plotTreeMapEmissions("World" ,  c("Cattle") , TRUE, 209,49)
    return (fig)
  ...
  # output$ProductionBarChartMono <- renderPlotly({
  #   # production2Plot.dt <- prod.dt[ ADM0_NAME == input$selectCountry & Animal %in% input$inSelectMonogastricsProduction &
  #   #                                     Output %in% input$selectProdOutputProduct,
  #   #                                 sum (value, na.rm = T), by = .(Animal,ProdSys )]
  #   fig <- plotBarChartProduction (input$selectCountry, input$inSelectMonogastricsProduction,input$selectProdOutputProductMono, "All")
  #   
  # })
  # 
  # output$ProductionBarChartRumin <- renderPlotly({
  #   fig <- plotBarChartProduction (input$selectCountry, input$inSelectRuminantsProduction,input$selectProdOutputProductRumin, input$inProdSelectAnimalsRuminantsHerdType)
  #   #inProdSelectAnimalsRuminantsHerdType)
  #   
  #   #fig <- plotBarChartProduction (input$selectCountry, input$inSelectRuminantsProduction,input$selectProdOutputProductRumin, input$inProdSelectAnimalsRuminantsHerdType)
  #   
  # })
  ...
  output$selRuminantHerdType <- renderUI({
    list(
      checkboxGroupInput("axisx", "",choices = c("Beef","Dairy","All"), selected = "All", inline  = T)
    )
  })
  output$selRuminantProdSys <- renderUI({
    list(
      checkboxGroupInput("axisasd", "",choices = ruminants, selected= ruminants, inline = T)
    )
  })
  ...
  # 
  # output$emissionPieChartMono <- renderPlotly({
  #   # anim <- subsetAnimalsFromInput (input$inAnimalSelect)
  #   #anim = subsetAnimalsFromInput( input$inAnimalSelect)
  #   # anim <- animalSelection
  #   input$emInGWPNO2
  #   isDirect = input$inCheckDirectOnly
  #   fig <- plotyPieChart( input$selectCountry, input$inSelectMonogastricsEmissions, 
  #                         isDirect, input$emInGWPNO2, input$emInGWPCH4)
  #   return (fig)
  #   
  #   
  # })
  
  # output$emissionPieChartRumin <- renderPlotly({
  #   # anim <- subsetAnimalsFromInput (input$inAnimalSelect)
  #   #anim = subsetAnimalsFromInput( input$inAnimalSelect)
  #   # anim <- animalSelection
  #   isDirect = input$inCheckDirectOnly
  #   fig <- plotyPieChart(input$selectCountry ,  input$inSelectRuminantsEmissions, 
  #                        isDirect,input$emInGWPNO2, input$emInGWPCH4)
  #   return (fig)
  #   
  #   
